...
// into other AI chats / journals / notes. Ported verbatim from the web's
// lib/dayReport.ts so the copy-day output is byte-identical across clients.
// Concise, scannable, no emoji, no calorie-shame framing.

import { parseItems, type Item, type Meal } from "./types";

// Backfill sentinel: server stamps a meal logged for a past day as the
// last second of that day. Pure check by clock components.
...
export function formatDayReport(meals: Meal[], date: Date): string {
  ...
  for (const m of meals) {
    const items: Item[] = parseItems(m.items_json);
